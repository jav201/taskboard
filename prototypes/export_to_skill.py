...
#: small-number spellout for the header prose below -- the header names the
#: count in words ("the {n_word} visual languages"), and a digit would read
#: fine too, but wouldn't match this file's own prose register. Keeping it
#: derived from `len(langs)` (never a literal) is the whole point: the count
#: that drifted to "ten" over eleven kits is exactly the bug this generator
...
HEADER = '''r"""languages.py -- the {n_word} visual languages, AS IMPLEMENTED.
...
WHAT EACH NAME MEANS
    ...
    ORDER      the {n_word} in the order the app's `t` key cycles them.
...
Reference frames for all {n_word} live in `assets/languages/` -- board and component
