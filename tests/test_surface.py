...
DISPLAY_LANGS = [n for n in ORDER if THEMES[n]["surface"] == "display"]


@pytest.mark.parametrize("name", DISPLAY_LANGS)
def test_the_display_legend_is_the_callers_mark_and_reaches_the_chrome(name):
    """L-33 / SCOPE's F-1, and the reason this is a PROPERTY test.

    A mutation test proves a token is READ; only a property test proves it is
    read CORRECTLY. `test_every_optional_argument_is_read_or_declared_refused`
    would go green the moment `display` did ANYTHING different with `label` —
    lettered it backwards, hashed it, drew one cell of it. What has to be true
    is narrower and is the whole finding: the caller's BINDING and the
    caller's WORD both arrive, intact and adjacent, in the `chrome` rows the
    compositor draws — not in `rows`, because a caller outside Textual (SCOPE
    is one) consumes `chrome` and never sees `rows`.

    `[7] SOURCE` is the shape of the defect: SCOPE's keymap spends `[1]` on
    its source cycle, and the kit was lettering `[1] DISPLAY` over it."""
    res = LG.kit(name).raster_region(probe(), W, H, label="7 SOURCE")
    top = Text.from_markup(res.chrome[0]).plain
    assert "[7] SOURCE" in top, (
        f"{name}: the caller's legend did not reach the chrome: {top!r}")


@pytest.mark.parametrize("name", DISPLAY_LANGS)
def test_an_unnumbered_display_drops_the_binding_it_has_no_notation_for(name):
    """L-33's TIE, asserted rather than assumed: *a commitment that ties a
    notation to a behaviour makes the notation unavailable to anything without
    that behaviour.*

    §3b numbers a display because the number IS a keybinding. A language whose
    `numbered` token is off has made no such commitment, so it has no notation
    for a binding — and it must not grow one out of a caller's string. The
    word survives; the bracket does not.

    SWAPPED RATHER THAN PICKED, and that is a correction to the acceptance as
    briefed (spec AC-2): the AC said industrial "is not `numbered`". It is.
    Both languages that use the `display` posture are numbered, so NO shipped
    language exercises this limb, and asserting it needed the same token swap
    `test_mutation_changes_the_render` already uses. A kit with `numbered`
    turned off is a real kit — `Kit.numbered` is `bool(self.t.get("numbered"))`
    — not a mock."""
    original = THEMES[name].get("numbered")
    try:
        THEMES[name]["numbered"] = False
        top = Text.from_markup(
            LG.kit(name).raster_region(probe(), W, H, label="7 SOURCE")
            .chrome[0]).plain
    finally:
        THEMES[name]["numbered"] = original

    assert "SOURCE" in top, f"{name}: the word was dropped too: {top!r}"
    assert "7" not in top and "[" not in top, (
        f"{name}: an unnumbered language lettered a binding it has no "
        f"notation for: {top!r}")


@pytest.mark.parametrize("name", DISPLAY_LANGS)
def test_a_legend_with_no_binding_keeps_the_languages_own_index(name):
    """The other half of the split rule, and the one the SWEEP exercises.

    `capture_languages.py` hands every language `SURFACE_LABEL = "mbb rho
    final"` — a word with no leading integer. The language must then keep its
    own index, so the shipped `surface_corgi.txt` still SHOWS that corgi is
    `numbered`. If a bare word silently reset the index the numbering token
    would stop being visible in the one artefact that documents it."""
    top = Text.from_markup(
        LG.kit(name).raster_region(probe(), W, H, label="mbb rho final")
        .chrome[0]).plain
    assert "[1] MBB RHO FINAL" in top, f"{name}: {top!r}"


@pytest.mark.parametrize("name", DISPLAY_LANGS)
def test_an_untold_display_still_says_what_it_always_said(name):
    """The commitment that keeps the other ten languages' frames still: told
    nothing, the posture renders exactly what it rendered before L-33 was
    fixed. The byte comparison against `.fast-dev-flow/baseline-kits2/` is the
    real gate; this states the intent in one line so a future edit that moves
    the default fails HERE, naming the reason, rather than as 62 mystery
    frame diffs."""
    top = Text.from_markup(
        LG.kit(name).raster_region(probe(), W, H).chrome[0]).plain
    assert "[1] DISPLAY" in top, f"{name}: the default legend moved: {top!r}"
