...
- **F-12** (new): `surfaces_index()` promises its table describes the frame it names and nothing
  checks it. Caught here in staging; not fixed **in this batch's four increments**.
  **CLOSED 2026-09-04 by `inc11.md`**, a standalone increment against this finding after §8 was
  first written: `check_box_matches_shipped()` now derives the shipped frame's glass rectangle and
  aborts the export on disagreement (`SurfaceIndexMismatch`), with a green-over-eleven test and a
  stale-frame mutant. Suite went 315 -> **327 passed, 2 skipped**. This line is the only amendment
  made to §8 after the fact, and it is dated for that reason.
